...
STRINGS[en,not_debian_based]="Error: Supported systems are Debian 11+, Ubuntu 20.04+, Fedora 43/44, Enterprise Linux 9/10, and Alpine Linux 3.24."
...
STRINGS[zh,not_debian_based]="错误：支持 Debian 11+、Ubuntu 20.04+、Fedora 43/44、Enterprise Linux 9/10 和 Alpine Linux 3.24。"
...
check_system() {
    ...
    case "$DISTRO_ID" in
        ...
        fedora)
            ...
            RPM_DIST_TAG="fc$DISTRO_VERSION_MAJOR"
            ;;
        rhel|rocky|centos|almalinux|ol)
            ...
            RPM_DIST_TAG="el$DISTRO_VERSION_MAJOR"
            ;;
        alpine)
            DISTRO_FAMILY=apk
            case "${VERSION_ID:-}" in
...
download_packages() {
    ...
    case "$DISTRO_FAMILY" in
        ...
        rpm)
            assets_json=$(jq -r \
                --arg arch "$RPM_ARCH" \
                --arg dist "$RPM_DIST_TAG" \
                '.assets[] | select(.name | test("^kernel-cloud-bbrv3(-devel)?-")) | select(.name | contains("." + $dist + ".")) | select(.name | endswith("." + $arch + ".rpm")) | .browser_download_url' \
                <<< "$release_json")
            ;;
        apk)
            assets_json=$(jq -r \
...
install_packages() {
    ...
    case "$DISTRO_FAMILY" in
        ...
        rpm)
            local rpm_packages=()
            mapfile -t rpm_packages < <(find "$DOWNLOAD_DIR" -name '*.rpm')
            if [ "${#rpm_packages[@]}" -eq 0 ]; then
                print_colored "${RED}" "$(get_string install_failed)"
                exit 1
            fi
            if ! run_as_root dnf install -y "${rpm_packages[@]}"; then
                print_colored "${RED}" "$(get_string install_failed)"
                exit 1
            fi
            ;;
        apk)
            ...
            mapfile -t apk_packages < <(find "$DOWNLOAD_DIR" -name '*.apk')
            if [ "${#apk_packages[@]}" -eq 0 ]; then
                print_colored "${RED}" "$(get_string install_failed)"
                exit 1
            fi
            if ! run_as_root apk add "${apk_packages[@]}"; then
                print_colored "${RED}" "$(get_string install_failed)"
                exit 1
            fi
            ;;
...
main() {
    ...
    # Check system compatibility before using its package manager
    check_system
    update_package_index
    check_arch
    install_dependency curl
