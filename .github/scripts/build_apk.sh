#!/usr/bin/env bash
# Build Alpine APKs from the shared prepared kernel archive.
set -euo pipefail

usage() {
  printf 'usage: %s <kernel-version> <arch> <alpine-id> <source-archive> <output-dir>\n' "$(basename "$0")" >&2
  exit 1
}
...
work_dir=$repo_root/.build/apk/$alpine_id-$arch_arg
...
apk add --no-cache \
  alpine-sdk bash bc binutils bison bpftool ccache diffutils doas elfutils-dev \
  findutils flex gawk git gmp-dev installkernel linux-firmware-any linux-headers \
  mkinitfs mpc1-dev mpfr-dev openssl-dev pahole perl python3 rsync sed tar xz \
  zstd
...
export CCACHE_DIR=${CCACHE_DIR:-$repo_root/.ccache/apk/$alpine_id-$arch_arg}
mkdir -p "$CCACHE_DIR"
